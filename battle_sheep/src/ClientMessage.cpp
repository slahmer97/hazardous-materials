...
std::string ClientMessage::getRotateMessage(int id,unsigned char clock) {
    boost::property_tree::ptree pt;
...
std::string ClientMessage::getChoseGridMessage(unsigned char id) {
    boost::property_tree::ptree pt;
...
ClientMessage* ClientMessage::getClientMessage(const std::string& json){
        ...
        if(msg_type == SHOT ||  msg_type == MOVE || msg_type == ROTATE || msg_type == CHOOSE_GRID){
            clientMessage->set_id( ptree.get<unsigned char>("id"));
            if(msg_type == SHOT){
...
void ClientMessage::set_login(const std::string & l) {
    m_login = l;
}
void ClientMessage::set_id(unsigned char id) {
    m_id = id;
}
...
unsigned char ClientMessage::get_id() {
    return m_id;
}
